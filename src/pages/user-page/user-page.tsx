...
import { postAppointmentRequest } from '../../services/api';
...
const UserPage: FC = () => {
  ...
  const ListItemLink = (props: ListItemLinkProps) => {
    ...
    return (
      <ListItem component={RouterLink} {...props} onClick={handleDrawerClose}>
        <Typography sx={{ width: '100%' }} textAlign="center" color={match ? 'secondary' : ''}>
          {props.title}
        </Typography>
      </ListItem>
    );
  ...
  const handleSelectTrainingChange = (evt: SelectChangeEvent<typeof trainingValue>) => {
    setTrainingValue(evt.target.value);
  };

  const handleSelectPointChange = (evt: SelectChangeEvent<typeof pointValue>) => {
  ...
  const handleFormSubmit = (evt: FormEvent<HTMLFormElement>) => {
    ...
    // fetch('https://script.google.com/macros/s/AKfycbyZS6fxaH-Cj4nVS2Vo3JkOl84PK2WkFQrhU0T6Aut8ow3MdrdsMpMEZTwiH5SeAcVY/exec', {
    //   method: 'POST',
    //   body: formData,
    // })
    //   .then(() => {
  ...
  useEffect(() => {
    ...
    if ([Routes.Children, Routes.Adults, Routes.Camps, Routes.Trainings].includes(location.pathname as Routes))
    ...
    <Container sx={{ p: { xs: 0 } }}>
      <AppBar position="sticky">
        <Toolbar sx={{ px: { xs: 4 }, justifyContent: 'space-between' }}>
          <IconButton sx={{ p: 0 }} size="small" aria-label="open drawer" onClick={handleDrawerOpen}>
      ...
      >
        ...
        <List component="nav" sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <ListItemButton onClick={handleSubListButtonClick}>
            ...
            {isSubListOpen ? <ExpandLessIcon sx={{ color: '#fff' }} /> : <ExpandMoreIcon sx={{ color: '#fff' }} />}
          </ListItemButton>
          <Collapse in={isSubListOpen} timeout="auto" unmountOnExit>
            <List disablePadding>
              <ListItemLink title="Индивидуальные тренировки" to={Routes.Trainings} />
      ...
      <main>
        ...
        <Container sx={{ py: { xs: 0, md: 0 } }}>
          ...
          <Box display="flex" flexWrap="wrap" alignItems="center" justifyContent="center" gap={2} pt={4} mb={8}>
            <Box component="img" sx={{ width: 160 }} src={tigerLogo}></Box>
            <Box
              component="img"
              sx={{ width: 160, height: 160, objectFit: 'cover', display: 'flex', justifyContent: 'center', alignItems: 'center' }}
              src={ftbmLogo}
            ></Box>
        ...
        <Container sx={{ pb: { xs: 0, md: 0 } }}>
          ...
          <Box display="flex" justifyContent="center" mb={8}>
            <a href="https://challengemuaythai.ru/" target="_blank">
        ...
        <Container sx={{ pb: { xs: 0, md: 0 } }}>
          ...
          <Box
            component="form"
            sx={{ mb: 8, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}
            onSubmit={handleFormSubmit}
          >
            <TextField fullWidth name="name" label="ФИО" required></TextField>
            <FormControl fullWidth variant="outlined">
              <InputLabel htmlFor="phone">Номер телефона *</InputLabel>
              <OutlinedInput id="phone" name="phone" inputComponent={TextMaskCustom as any} label="Номер телефона" required />
            </FormControl>
            <TextField fullWidth type="email" name="email" label="Почта" required></TextField>
            ...
            <TextField fullWidth name="whence" label="Откуда о нас узнали" required></TextField>
            <FormControl fullWidth>
              ...
              >
                ...
                <MenuItem value="для поддержания физической формы">Для поддержания физической формы</MenuItem>
                <MenuItem value="для выступления на соревнованиях">Для выступления на соревнованиях</MenuItem>
              </Select>
            </FormControl>
            <Button sx={{ width: '160px' }} type="submit" color="secondary" variant="contained">
              Записаться
            </Button>
          </Box>
          <Modal
      ...
      <footer>
        <Container>
          ...
          <Typography mb={4}>Почта: topfightgym@mail.ru</Typography>
          <Typography variant="h4" mb={2}>
            Следите за нами в
          </Typography>
          <MuiLink sx={{ display: 'flex', alignItems: 'center', gap: 1 }} mb={2} href="https://vk.com/club220739349" target="_blank">
