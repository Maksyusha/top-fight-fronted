...
const ImageListItemLink: FC<{
  to: string;
  src: string;
  title: string;
  isMatchedUp: boolean;
  index: number;
}> = ({ to, src, title, isMatchedUp, index }) => {
  const cols = isMatchedUp ? 1 : index === 0 ? 2 : 1;
  const rows = isMatchedUp ? 1 : index === 0 ? 2 : 1;

  return (
    <ImageListItem component={Link} to={to} cols={cols} rows={rows}>
...
const HomePage: FC = () => {
  ...
  const { images } = useAppSelector((store) => store.gallery);
        ...
        <ImageList sx={{ m: 0 }} variant="quilted" cols={2} gap={4}>
          {imageListData.map(({ to, src, title }, index) => (
            <ImageListItemLink
              ...
              src={src}
              title={title}
              isMatchedUp={isMatchedUp}
              index={index}
            />
          ))}
        </ImageList>
      </Container>
      <Divider sx={{ mx: { xs: 4, sm: 8 } }} />
      <Container
